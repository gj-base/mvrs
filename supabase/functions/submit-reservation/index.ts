...
Deno.serve(async (req: Request) => {
  ...
  const [slotCheckResult, timeBlockResult] = await Promise.all([
    sb
      .from("reservations")
      .select(
  ...
  const { data: slotCheckRows, error: slotCheckErr } = slotCheckResult;
  if (slotCheckErr) {
    return json(200, {
      ok: false,
  ...
  if (timeBlockResult.error) {
    return json(200, {
      ok: false,
      error: "환입 제한 시간 확인 오류: " + timeBlockResult.error.message,
    });
  }
  ...
  if (adminTimeBlock) {
    return json(200, { ok: false, error: adminTimeBlockErrorMessage(adminTimeBlock) });
  }
