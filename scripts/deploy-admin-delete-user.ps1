# Edge Function admin-delete-user 배포
...
if (-not $env:SUPABASE_ACCESS_TOKEN) {
  ...
  Write-Host "  .\scripts\deploy-admin-delete-user.ps1"
  exit 1
}

& $supabaseExe functions deploy admin-manage-public-settings --no-verify-jwt --project-ref $projectRef
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
Write-Host "배포 완료: admin-manage-public-settings (delete_user 포함)"

& $supabaseExe functions deploy admin-delete-user --no-verify-jwt --project-ref $projectRef
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
Write-Host "배포 완료: admin-delete-user"

# admin-list-user-signups 도 is_master 필드 추가됨 — 함께 재배포
& $supabaseExe functions deploy admin-list-user-signups --no-verify-jwt --project-ref $projectRef
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
Write-Host "배포 완료: admin-list-user-signups"
